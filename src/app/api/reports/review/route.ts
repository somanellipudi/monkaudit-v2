...
import { reviewReport } from "@/lib/server/repositories";
...
export async function POST(request: Request) {
  const auth = await requirePermission(request, "report:approve");
  ...
  const report = await reviewReport(payload.auditRunId, payload.type, auth.user.email);
  if (!report) {
    return NextResponse.json({ error: "Report not found. Save a draft before review." }, { status: 404 });
  }
